    ...
    <div class="d-flex flex-column h-100">
      <v-list class="flex-grow-1 overflow-y-auto">
        ...
        <v-hover v-slot="{ isHovering, props }" 
                v-for="(item, index) in controlsStore.notifications" :key="index">
            ...
            <v-list-item :title="item.fileName" :subtitle="item.fileSize" v-bind="props">
              ...
              <template v-slot:prepend>
                <v-list-item-action :start="true">
                  <v-avatar density="compact" :image="webrtcStore.getAvatar(item.channelName)"></v-avatar>
                </v-list-item-action>
              </template>
              <template v-slot:append>
                <v-list-item-action v-if="isHovering">
                  <v-btn icon flat size="small" @click="acceptNotification(index)"><v-icon>mdi-check-bold</v-icon></v-btn>
                  <v-btn icon flat size="small" @click="controlsStore.popNotification(index)"><v-icon>mdi-cancel</v-icon></v-btn>
                </v-list-item-action>
              </template>
            </v-list-item>
          </v-expand-x-transition>
