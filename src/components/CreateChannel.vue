<template>
  <v-dialog v-model="controlsStore.createChannel" width="400" persistent="true">
    <template v-slot:activator>
      <v-btn icon @click="controlsStore.showCreateChannel"><v-icon>mdi-plus</v-icon></v-btn>
    </template>
    <v-card title="Create Channel">
          ...
          <v-col cols="12" class="d-flex flex-column">
           <div class="d-flex justify-center">
            ...
            <v-text-field label="Channel name" variant="solo-filled"
                          :rules="channelNameRules" :readonly="!!channelQRCode"
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn variant="text" @click="controlsStore.hideCreateChannel">cancel</v-btn>
        <v-btn variant="text" :disabled="isDisabled" @click="initConnection">create</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
